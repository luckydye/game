...
fn move_paddle(
    ...
    for (mut paddle_transform, mut paddle) in &mut query {
        paddle.direction = 0.0;
        ...
        let mut new_x = paddle_transform.translation.x
            + paddle.direction * PADDLE_SPEED * time_step.period.as_secs_f32();
...
fn check_ball_collisions(
    ...
    for (mut ball_velocity, ball_transform, ball) in &mut ball_query {
        for (other_entity, transform, other, opt_paddle, opt_brick) in &mut collider_query {
            ...
            if let Some(collision) = collision {
                ...
                if let Some(paddel) = opt_paddle {
                    ...
                    if reflect_y {
                        ball_velocity.y *= -1.;
                        ball_velocity.x += paddel.direction * 400.0;
                    }
                } else {
                    if reflect_x {
                        ball_velocity.x *= -1.;
